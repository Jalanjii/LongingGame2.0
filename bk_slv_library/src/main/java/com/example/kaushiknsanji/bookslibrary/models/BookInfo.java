...
public class BookInfo implements Parcelable {
    ...
    public String getPublishedDateInMediumFormat(String fallback) throws ParseException {
        if (!TextUtils.isEmpty(mPublishedDateStr)) {
            ...
            if (noOfDateParts == 1) { //When only the Year part is present
                return mPublishedDateStr; //Returning the Year AS-IS
            } else if (noOfDateParts == 2) { //When only the Year + Month part is present
            ...
            } else if (noOfDateParts == 3) { //When Complete date is present
